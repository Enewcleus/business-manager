...
router.post('/login', async (req, res) => {
  ...
  if (error) return res.json({ success: false, message: 'DB error: ' + error.message });
  ...
  res.json({ success: true, token, user: { userId: user.user_code, name: user.name, email: user.email, role: user.role, assignedClients: user.assigned_clients || [] } });
});
